...
tstart=0
over=0
w=[[7,6,5,4],[6,5,4,3],[5,4,3,2],[4,3,2,1]]
...
class PlayerAI(BaseAI):
        ...
        for i,x in enumerate(range(grid.size)):
            ...
            for j,y in enumerate(range(grid.size)):
                if (y<grid.size-1) and (x<grid.size-1):
                    b=[score_map[grid.map[x][y]],score_map[grid.map[x][y+1]],score_map[grid.map[x+1][y]],score_map[grid.map[x+1][y+1]]]
                    c=c + min(0,b[0]-b[1])  +  min(0,b[0]-b[2]) + 0.3*min(0,b[0]-b[3])
                elif (y<grid.size-1):
 ...
 #       self.adj=adjacent(grid)
        return self.avcells*3+self.adj+self.maxtile*20+self.mon*1.5#**2+self.mon/25#+self.edge#self.avcells*3+self.adj*1#+monotone2(state)/100#+edges(state)/100#+getmax(state)*400#+edges(state)/10#+edges(state)/5#+getmax(state)*400+monotone2(state)/3
...
#        tstart=time.clock()
        ...
        while (time.clock()-self.tstart<time_limit):
            results=self.maximize(grid,-10**6,10**6,0)
            if (results[2])==None: break
            moves.append(results[2])
            utility.append(results[1])
        ...
        if (len(moves)==1): self.dir=moves[0]
            ...
            moves=moves[-2:]
            utility=utility[-2:]
            self.dir=moves[utility.index(max(utility))]
